...
function getEmployeur() { const p = getProfilEntreprise(); return { nom: p.nom || 'Mon Entreprise', siret: p.siret || '—', adresse: `${p.adresse || ''} ${p.codePostal || ''} ${p.ville || ''}`.trim() || '—', rcs: '—', assuranceDecennale: p.decennale || '—', rcPro: p.rcpro || '—', representant: p.signatureGerant || '—', fonction: 'Gérant' }; }

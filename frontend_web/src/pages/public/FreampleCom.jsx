...
// ── Palette luxe ──
const L = {
  bg: '#FAFAF8', white: '#FFFFFF', noir: '#0A0A0A',
...
function useReveal() {
  ...
  useEffect(() => {
    ...
    el.style.transition = 'opacity .8s cubic-bezier(0.25,0.46,0.45,0.94), transform .8s cubic-bezier(0.25,0.46,0.45,0.94)';
...
export default function FreampleCom() {
  ...
  useEffect(() => {
    api.get('/com/tarifs').then(r => { if (r.data.tarifs) setTarifs(r.data.tarifs); if (r.data.packs) setPacks(r.data.packs); }).catch(() => {});
  }, []);
  ...
  const MENU_ITEMS = [
    ...
    { label:'Formules', id:'tarifs' },
    { label:'Tarifs', id:'grille' },
    ...
    <div style={{ minHeight:'100vh', background:L.bg, fontFamily:L.font, color:L.text }}>
      ...
      <section style={{ background:L.noir, padding:'clamp(80px,14vh,140px) 32px clamp(72px,12vh,120px)', textAlign:'center', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:720, margin:'0 auto', position:'relative', zIndex:1 }}>
          ...
          <h1 style={{ fontFamily:L.serif, fontSize:'clamp(38px,7vw,72px)', fontWeight:300, fontStyle:'italic', color:'#fff', lineHeight:1.05, letterSpacing:'-0.02em', margin:'0 0 20px' }}>
            L'excellence du<br/><span style={{ fontWeight:700, fontStyle:'normal' }}>montage vidéo</span>
          </h1>
          <p style={{ fontSize:'clamp(15px,1.8vw,18px)', color:'rgba(255,255,255,0.45)', lineHeight:1.65, margin:'0 auto 44px', maxWidth:460, fontWeight:300 }}>
            Misez sur le digital pour des résultats réels.
          </p>
          <div style={{ display:'flex', gap:16, justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={()=>setStep(1)} style={{ padding:'16px 44px', background:L.white, color:L.noir, border:'none', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:L.font, letterSpacing:'0.06em', textTransform:'uppercase', transition:'all .3s' }}
      ...
      {/* ══ MARQUEE ══ */}
      <div style={{ background:L.noir, borderBottom:`1px solid rgba(255,255,255,0.06)`, overflow:'hidden', padding:'14px 0' }}>
        <div style={{ display:'flex', animation:'marquee 25s linear infinite', whiteSpace:'nowrap' }}>
          {[...Array(3)].map((_, k) => (
            <span key={k} style={{ display:'flex', gap:0 }}>
              {['Montage vidéo','TikTok','YouTube','Reels','Design','Branding','Stratégie','Création','Excellence','Innovation'].map(w => (
                <span key={w+k} style={{ fontSize:12, color:'rgba(255,255,255,0.2)', fontWeight:500, textTransform:'uppercase', letterSpacing:'0.15em', padding:'0 28px' }}>
      ...
      {/* ══ SAVOIR-FAIRE — Section split ══ */}
      <section id="expertises" style={{ scrollMarginTop:20 }}>
        <div ref={r1} style={{ display:'flex', flexWrap:'wrap', maxWidth:1200, margin:'0 auto' }}>
          {/* Left — Image */}
          <div style={{ flex:'1 1 400px', minHeight:400, background:`url(https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?w=800&q=80) center/cover`, position:'relative' }}>
            <div style={{ position:'absolute', inset:0, background:'rgba(10,10,10,0.3)' }} />
          </div>
          {/* Right — Content */}
          <div style={{ flex:'1 1 400px', padding:'clamp(40px,6vh,72px) clamp(32px,4vw,56px)', display:'flex', flexDirection:'column', justifyContent:'center', background:L.white }}>
            ...
            <h2 style={{ fontFamily:L.serif, fontSize:'clamp(28px,3.5vw,42px)', fontWeight:300, fontStyle:'italic', letterSpacing:'-0.02em', margin:'0 0 24px', lineHeight:1.15 }}>
              Tout ce dont vos <span style={{ fontWeight:700, fontStyle:'normal' }}>réseaux</span> ont besoin
            </h2>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
              {[
                { icon:'🎬', title:'Montage vidéo' },
                { icon:'🎨', title:'Direction artistique' },
                { icon:'📱', title:'Gestion réseaux' },
                { icon:'💬', title:'Sous-titres & SFX' },
                { icon:'📈', title:'Stratégie digitale' },
                { icon:'🎙️', title:'Production contenu' },
              ].map(s => (
                <div key={s.title} style={{ display:'flex', alignItems:'center', gap:12 }}>
                  <span style={{ fontSize:20 }}>{s.icon}</span>
                  <span style={{ fontSize:14, fontWeight:600, color:L.text }}>{s.title}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ══ CHIFFRES ══ */}
      <section ref={r2} style={{ background:L.noir, padding:'clamp(52px,8vh,80px) 32px' }}>
        <div style={{ maxWidth:800, margin:'0 auto', display:'flex', justifyContent:'center', gap:'clamp(40px,8vw,100px)', flexWrap:'wrap', textAlign:'center' }}>
          {[
            ...
            <div key={s.val}>
              <div style={{ fontFamily:L.serif, fontSize:'clamp(32px,5vw,52px)', fontWeight:300, color:L.gold, letterSpacing:'-0.03em', lineHeight:1 }}>{s.val}</div>
              <div style={{ fontSize:11, color:'rgba(255,255,255,0.35)', marginTop:10, textTransform:'uppercase', letterSpacing:'0.15em', fontWeight:500 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ══ PORTFOLIO CTA ══ */}
      <section ref={r3} id="portfolio" style={{ background:L.noir, padding:'clamp(72px,12vh,110px) 32px', textAlign:'center', position:'relative', overflow:'hidden', scrollMarginTop:20 }}>
        <div style={{ position:'absolute', inset:0, background:'linear-gradient(135deg, rgba(201,169,110,0.04) 0%, transparent 50%, rgba(201,169,110,0.02) 100%)' }} />
        <div style={{ position:'relative', zIndex:1, maxWidth:600, margin:'0 auto' }}>
          ...
          <h2 style={{ fontFamily:L.serif, fontSize:'clamp(28px,5vw,50px)', fontWeight:300, fontStyle:'italic', color:'#fff', letterSpacing:'-0.02em', lineHeight:1.1, margin:'0 0 16px' }}>
            Découvrez nos <span style={{ fontWeight:700, fontStyle:'normal' }}>réalisations</span>
          </h2>
          <p style={{ fontSize:15, color:'rgba(255,255,255,0.4)', lineHeight:1.6, margin:'0 0 40px', fontWeight:300 }}>
        ...
        <div style={{ position:'absolute', top:'50%', left:0, width:80, height:1, background:`linear-gradient(90deg, transparent, ${L.gold}25)` }} />
        <div style={{ position:'absolute', top:'50%', right:0, width:80, height:1, background:`linear-gradient(90deg, ${L.gold}25, transparent)` }} />
      </section>

      {/* ══ MARQUEE 2 ══ */}
      <div style={{ background:L.bg, borderTop:`1px solid ${L.border}`, borderBottom:`1px solid ${L.border}`, overflow:'hidden', padding:'12px 0' }}>
        <div style={{ display:'flex', animation:'marquee2 30s linear infinite', whiteSpace:'nowrap' }}>
          {[...Array(3)].map((_, k) => (
            <span key={k} style={{ display:'flex' }}>
              {['Qualité','Créativité','Rigueur','Passion','Expertise','Engagement','Innovation','Authenticité'].map(w => (
                <span key={w+k} style={{ fontFamily:L.serif, fontSize:16, fontStyle:'italic', color:L.textLight, padding:'0 32px', fontWeight:400 }}>
                  {w} <span style={{ color:L.gold, margin:'0 8px', fontStyle:'normal' }}>—</span>
                </span>
              ))}
            </span>
          ))}
      ...
      {/* ══ CONTACT — Split inversé ══ */}
      <section ref={r4} id="contact" style={{ scrollMarginTop:20 }}>
        <div style={{ display:'flex', flexWrap:'wrap', maxWidth:1200, margin:'0 auto' }}>
          <div style={{ flex:'1 1 400px', padding:'clamp(48px,7vh,80px) clamp(32px,4vw,56px)', display:'flex', flexDirection:'column', justifyContent:'center', background:L.white }}>
            ...
            <h2 style={{ fontFamily:L.serif, fontSize:'clamp(26px,3.5vw,40px)', fontWeight:300, fontStyle:'italic', color:L.text, letterSpacing:'-0.02em', lineHeight:1.15, margin:'0 0 12px' }}>
              Vous avez un projet ?<br/><span style={{ fontWeight:700, fontStyle:'normal' }}>Parlons-en.</span>
            </h2>
            <p style={{ fontSize:14, color:L.textSec, lineHeight:1.6, margin:'0 0 28px' }}>Réponse sous 24 heures, devis gratuit et sans engagement.</p>
          ...
          <div style={{ flex:'1 1 400px', minHeight:360, background:`url(https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=800&q=80) center/cover`, position:'relative' }}>
      ...
      {/* ══ FORMULES ══ */}
      <section ref={r5} id="tarifs" style={{ background:L.white, borderTop:`1px solid ${L.border}`, padding:'clamp(60px,9vh,96px) 32px', scrollMarginTop:20 }}>
        <div style={{ maxWidth:960, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:52 }}>
            ...
            <h2 style={{ fontFamily:L.serif, fontSize:'clamp(28px,4vw,44px)', fontWeight:300, fontStyle:'italic', letterSpacing:'-0.02em', margin:'0 0 8px', lineHeight:1.15 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px, 1fr))', gap:0, border:`1px solid ${L.border}` }}>
            {packs.map((pack, i) => {
              ...
              return (
                <div key={i} style={{ background:isPop?L.noir:L.white, padding:'40px 28px', display:'flex', flexDirection:'column', borderRight:i<packs.length-1?`1px solid ${isPop?'rgba(255,255,255,0.08)':L.border}`:'none', position:'relative' }}>
                  ...
                  <div style={{ fontFamily:L.serif, fontSize:42, fontWeight:300, color:isPop?'#fff':L.text, marginBottom:4, letterSpacing:'-0.03em' }}>
                    {pack.prix}€<span style={{ fontSize:14, fontWeight:400, color:isPop?'rgba(255,255,255,0.3)':L.textLight, fontFamily:L.font }}>/mois</span>
                  </div>
                  <div style={{ fontSize:13, color:isPop?'rgba(255,255,255,0.35)':L.textSec, marginBottom:24 }}>{pack.desc}</div>
                  <div style={{ flex:1 }} />
      ...
      {/* ══ GRILLE PRIX ══ */}
      <section ref={r6} id="grille" style={{ background:L.bg, padding:'clamp(52px,7vh,80px) 32px', borderTop:`1px solid ${L.border}`, scrollMarginTop:20 }}>
        <div style={{ maxWidth:600, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:36 }}>
            ...
            <h3 style={{ fontFamily:L.serif, fontSize:28, fontWeight:300, fontStyle:'italic', margin:0, letterSpacing:'-0.02em' }}>Tous nos <span style={{ fontWeight:700, fontStyle:'normal' }}>tarifs</span></h3>
          </div>
          {tarifs.filter(t => t.cat === 'Montage vidéo').map(t => (
            <div key={t.cat} style={{ background:L.white, border:`1px solid ${L.border}` }}>
              <div style={{ padding:'16px 20px', borderBottom:`1px solid ${L.border}`, fontSize:14, fontWeight:600, color:L.text, letterSpacing:'-0.01em' }}>{t.cat}</div>
              {t.items.map((item,j) => (
                <div key={j} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 20px', borderBottom:j<t.items.length-1?`1px solid ${L.borderLight}`:'none', fontSize:14, gap:12 }}>
          ...
          <div style={{ background:L.white, width:'100%', maxWidth:520, maxHeight:'90vh', overflowY:'auto', padding:'36px 32px' }} onClick={e=>e.stopPropagation()}>
            ...
            {step===1 && (<div>
              <div style={{ marginBottom:20 }}>
                ...
                <select value={brief.style} onChange={e=>setBrief(p=>({...p,style:e.target.value}))} style={{...inp}}><option value="">Non précisé</option><option>Dynamique</option><option>Minimaliste</option><option>Fun / Décalé</option><option>Pro / Corporate</option><option>Cinématique</option></select>
              </div>
              <div style={{ marginBottom:20 }}>

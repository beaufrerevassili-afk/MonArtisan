  ...
  return (
    <div style={{ minHeight:'100vh', background:L.bg, fontFamily:L.font, color:L.text }}>
      ...
      <section style={{ background:L.noir, padding:'clamp(80px,14vh,140px) 32px', textAlign:'center', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', inset:0, backgroundImage:'url(https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=1600&q=80)', backgroundSize:'cover', backgroundPosition:'center', opacity:0.2 }} />
      ...
      <section style={{ padding:'clamp(56px,8vh,88px) 32px', maxWidth:900, margin:'0 auto' }}>
        ...
        <StaggerChildren style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(240px, 1fr))', gap:1, background:L.border }}>
          {[
            ...
            { icon:'🛡️', title:'Protection juridique', desc:'Assurance et accompagnement en cas de litige, contentieux ou procédure judiciaire.' },
            { icon:'🏢', title:'Droit des entreprises', desc:'Création de société, statuts, pactes d\'associés, modification de structure.' },
            { icon:'🏠', title:'Droit immobilier', desc:'Litiges locatifs, copropriété, vices cachés, permis de construire.' },
            { icon:'👷', title:'Droit du travail', desc:'Licenciement, contrats, prud\'hommes, harcèlement — vos droits expliqués.' },
          ].map(s => (

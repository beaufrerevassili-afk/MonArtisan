      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '24px clamp(16px,4vw,40px)' }}>
        ...
        {projetDetail && (() => {
          ...
          return <div>
            ...
            {/* ══ MESSAGERIE PROJET + DEVIS (vrais comptes) ══ */}
            {!isDemo && p.statut === 'publie' && offres.length > 0 && (() => {
              const [projetMessages, setProjetMessages] = React.useState([]);
              const [projetDevis, setProjetDevis] = React.useState([]);
              const [msgInput, setMsgInput] = React.useState('');
              const msgBottomRef = React.useRef(null);

              React.useEffect(() => {
                const load = async () => {
                  try {
                    const [mRes, dRes] = await Promise.all([
                      api.get(`/marketplace/projets/${p.id}/messages`),
                      api.get(`/marketplace/projets/${p.id}/devis`),
                    ]);
                    if (mRes.data?.messages) setProjetMessages(mRes.data.messages);
                    if (dRes.data?.devis) setProjetDevis(dRes.data.devis);
                  } catch {}
                };
                load();
              ...
              React.useEffect(() => { msgBottomRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [projetMessages]);

              const envoyerMsg = async () => {
                if (!msgInput.trim()) return;
                await api.post(`/marketplace/projets/${p.id}/messages`, { message: msgInput });
                setMsgInput('');
                const { data } = await api.get(`/marketplace/projets/${p.id}/messages`);
                if (data?.messages) setProjetMessages(data.messages);
              };

              const accepterDevis = async (devisId) => {
                await api.put(`/marketplace/devis/${devisId}/accepter`);
                addToast('Devis accepté — le chantier est créé !', 'success');
                chargerProjets();
                setProjetDetail(null);
              };
              ...
              const contesterDevis = async (devisId) => {
                ...
                const { data } = await api.get(`/marketplace/projets/${p.id}/messages`);
                if (data?.messages) setProjetMessages(data.messages);
                const { data: dData } = await api.get(`/marketplace/projets/${p.id}/devis`);
                if (dData?.devis) setProjetDevis(dData.devis);
              };

              return (
                <div style={{ marginBottom: 16 }}>
                  {/* Devis versionnés */}
                  {projetDevis.length > 0 && (
                  ...
                  <div style={CARD}>
                    ...
                    <div style={{ maxHeight: 300, overflowY: 'auto', marginBottom: 10 }}>
                      {projetMessages.length === 0 && <div style={{ textAlign: 'center', color: '#555', fontSize: 12, padding: 16 }}>Discutez avec l'artisan avant d'accepter</div>}
                      {projetMessages.map(m => {
